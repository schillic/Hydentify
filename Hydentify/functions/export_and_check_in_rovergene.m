function [ answer ] = export_and_check_in_rovergene(exists, transition_relation, transient_set)
%export_and_check_in_rovergene - export Kripke Structures and check property
%
...
% Christian: I only modified the function header for separate analysis of exists
% and forall Kripke structures.
...
if (exists)
    ...
    if length(transient_set)~= prod(box_nb_i); %if not all states are transient
        % write export file
        export_KS(transition_relation, transient_set, nusmv_file_name.existsP);
        % check whether property is true, using NuSMV
        answer = check_property(nusmv_file_name.existsP);
    else 
        %GB, Nov 7, 2010: change here
        'Warning: all states are transients'
        answer = 1; %the property vacuously holds
    end
%     output(['result_existP = ' num2str(answer) '\n']);

else
    %------------------ forallP semantics ----------------------
    % We use "live for all parameter" means "not transient for some parameters"
    if length(transient_set)~= prod(box_nb_i); %if they are not all transient
        % write export file
        export_KS(transition_relation, transient_set, nusmv_file_name.forallP);
        % check whether property is true, using NuSMV
        answer = check_property(nusmv_file_name.forallP);
    else 
        answer = 1;
    end
%     output(['result_forallP = ' num2str(answer) '\n']);
end
